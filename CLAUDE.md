...
- **Version bumps**: do *not* bump `SETTINGS_VERSION` for a preset change; no default is moving. Each saved preset is stamped with the `_v` it was captured at, and on a future bump the active id falls back to `standard` while the library is left intact — otherwise a stale user preset stays applied over freshly retuned defaults. That check is a **one-shot effect on mount**, not a per-render rule: applied per render it would also answer a *deliberate* re-pick of that preset with "Standard, modified", which is exactly the lie the derived `dirty` flag exists to prevent.
...
- **Live switching**: every preset key is applied without restarting the AudioContext, `windowSize` included — the loop reallocates the `Detector` and buffer when it changes (`usePitchDetector.js`). A window change still resets the tracker, since the held note and the median buffer were measured through the old window, but the learned room floor is now carried across: the RMS of stationary noise barely depends on window length, and relearning it would have meant tapping Standard → Reactive → Standard ran the two presets against three different noise gates. That difference reads as "Reactive is more sensitive", which is the comparison, not a detail. The held note itself does survive a switch that leaves `windowSize` alone, so the first `holdMs` after one is a hybrid.
